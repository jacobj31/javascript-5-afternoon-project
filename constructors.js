...
Movie.prototype.changeRating = function(num){
  var newrating = ((num + this.rating)/2) 
...
User.prototype.addSavedPost = function(id, title, rating){
  var newpost = {id:id, title:title, rating:rating}
...
User.prototype.removeSavedPost = function(num){
  target = this.savedPosts.indexOf(post => post.id === num)
  this.savedPosts.splice(target, 1)
}
...
User.prototype.changePostRating = function(id, newrating){
  let target = this.savedPosts.findIndex(post => post.id === id)
  this.savedPosts[target].rating = newrating
}
